...
// requests. Like Handle, but has a third parameter for the values of
...
// HandlerFunc is an adapter which allows the usage of an Handle as a
// request handle.
...
func (r *Router) Lookup(method, path string) (Handle, map[string]string, bool) {
	...
	setPathValue := func(name, value string) {
		if p == nil {
	...
	if root := r.trees[method]; root != nil {
		handle, tsr := root.getValue(path, setPathValue)
		if handle == nil {
			return nil, nil, tsr
		}
		if p == nil {
...
func (r *Router) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if root := r.trees[req.Method]; root != nil {
		if handle, tsr := root.getValue(path, req.SetPathValue); handle != nil {
			handle(w, req)
			return
